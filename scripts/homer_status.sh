#!/bin/zsh
# homer_status.sh — quick health snapshot for HomerTrakker
# Usage: ./scripts/homer_status.sh
...
# Colors
B=$'%{[1m%}'; R=$'%{[31m%}'; G=$'%{[32m%}'; Y=$'%{[33m%}'; C=$'%{[36m%}'; Z=$'%{[0m%}'
...
POST_CT=0
if [[ -d "$POSTS_DIR" ]]; then
  POST_CT=$(ls -1 "$POSTS_DIR"/tonights_homer_*.txt 2>/dev/null | wc -l | tr -d ' ')
fi
...
if [[ -z "$REQ" ]]; then
  # try to infer from last nohup line
